...
void * thr(void * ptr)
{
	...
	cout<<size;
	int n;
	while((n=fread(bu,sizeof(char),45,f1))>0&&size>0)
	{
		cout<<bu[0];
...
int main()
{
    ...
    pthread_t t4;
    pthread_t t5;
    int port1=80808,port2=80807,port3=80806
    //string p=argv[1];
    //int port=atoi(argv[1]);
pthread_create(&t1,0,server,(void *)NULL);

pthread_create(&t2,0,client,(void *)port1);
pthread_create(&t3,0,client,(void *)port2);
pthread_create(&t4,0,client,(void *)port3);
